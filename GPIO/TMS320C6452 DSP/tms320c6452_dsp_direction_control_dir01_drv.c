...
{
    tms320c6452_dir_mb();
    *addr = v; /* Big-endian access assumed by environment */
    tms320c6452_dir_mb();
}

/* Local helpers */
static int check_ctx(const tms320c6452_dsp_dir_ctx_t *ctx)
{
    return (ctx != NULL && ctx->ready == 1u) ? 1 : 0;
}

static int check_pin(uint32_t pin)
{
    return (pin < 32u) ? 1 : 0;
}
...
int tms320c6452_dsp_direction_control_dir01_init(tms320c6452_dsp_dir_ctx_t *ctx, uintptr_t base)
{
    ...
    ctx->ready = 1u;
    /* No side effects on hardware here; feature-only init */
...
int tms320c6452_dsp_direction_control_dir01_set_input(tms320c6452_dsp_dir_ctx_t *ctx, uint32_t pin)
{
    ...
    if (!check_ctx(ctx)) { return TMS320C6452_DSP_DIR_ESTATE; }
    if (!check_pin(pin)) { return TMS320C6452_DSP_DIR_EINVAL; }

    dir_reg = (volatile uint32_t *)((uintptr_t)ctx->base + TMS320C6452_DSP_GPIO_DIR01_OFFSET);
...
int tms320c6452_dsp_direction_control_dir01_get(tms320c6452_dsp_dir_ctx_t *ctx, uint32_t pin, uint32_t *is_input)
{
    ...
    if (!check_ctx(ctx) || is_input == NULL) { return TMS320C6452_DSP_DIR_EINVAL; }
    if (!check_pin(pin)) { return TMS320C6452_DSP_DIR_EINVAL; }
...
int tms320c6452_dsp_direction_control_dir01_read_mask(tms320c6452_dsp_dir_ctx_t *ctx, uint32_t *dir_mask)
{
    ...
    if (!check_ctx(ctx) || dir_mask == NULL) { return TMS320C6452_DSP_DIR_EINVAL; }
