...
/* UNKNOWN sentinel for offsets/stride when not available */
#define TMS320C6452_DSP_GPIO_REG_UNKNOWN ((uintptr_t)(~(uintptr_t)0))

/* Optional register offset placeholders (relative to base). Keep UNKNOWN if not specified. */
#define TMS320C6452_DSP_GPIO_BANK_STRIDE_OFFSET  TMS320C6452_DSP_GPIO_REG_UNKNOWN /* UNKNOWN */
#define TMS320C6452_DSP_GPIO_DIR_OFFSET          TMS320C6452_DSP_GPIO_REG_UNKNOWN /* UNKNOWN: Direction per bank */
...
/* Initialization */
tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_init(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    const tms320c6452_dsp_gpio_banked_cfg_t* cfg);

/* Bank-level APIs */
tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_config_dir_bank(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    uint32_t bank,
    uint32_t mask,
    bool is_output);

/* Set and clear bits within a bank; prefers atomic set/clear if available */
tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_write_bank_mask(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    uint32_t bank,
    uint32_t set_mask,
    uint32_t clear_mask);

/* Read a bank's current value (input preferred; falls back to output if input UNKNOWN) */
tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_read_bank(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    uint32_t bank,
    uint32_t* mask);

/* Pin-level helpers */
tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_write_pin(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    uint32_t pin,
    bool value);

tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_read_pin(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    uint32_t pin,
    bool* value);
...
tms320c6452_dsp_gpio_status_t tms320c6452_dsp_gpio_banked_irq_clear_bank(
    tms320c6452_dsp_gpio_banked_ctx_t* ctx,
    uint32_t bank,
    uint32_t clear_mask);
