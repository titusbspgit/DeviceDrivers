#include "tms320c6452_dsp_gpio_banked_arch_32x2_drv.h"
...
int32_t gpio_set_dir_bank(uint32_t bank, uint32_t mask, bool output)
{
    ...
    /* TMS320C6452: DIR bit 0=output, 1=input */
    if (output) {
        dir &= ~mapped;
    } else {
        dir |= mapped;
...
int32_t gpio_irq_enable_bank(uint32_t bank, uint32_t mask, bool enable)
{
    ...
    if (enable) {
        /* Enable both rising and falling edges for requested bits; enable bank master */
        ...
        /* Bank master left unchanged intentionally */
    }
