...
/* Base address from global configuration; can be overridden via init() */
...
/* Bitfield helpers */
#define TMS320C6452_DSP_GPIO_DIR_PIN_MASK(pin)   (1u << (uint32_t)(pin))
...
/* Driver context (feature-local, no dynamic allocation) */
typedef struct {
    uintptr_t base;   /* MMIO base for GPIO */
    uint32_t  ready;  /* 1 after init */
} tms320c6452_dsp_dir_ctx_t;

/* API: Initialize feature driver with a base address (use DEFAULT if base==0) */
...
/* API: Configure a pin as input (writes 1 to DIR01 bit) */
int tms320c6452_dsp_direction_control_dir01_set_input(tms320c6452_dsp_dir_ctx_t *ctx, uint32_t pin);

/* API: Configure a pin as output (writes 0 to DIR01 bit) */
