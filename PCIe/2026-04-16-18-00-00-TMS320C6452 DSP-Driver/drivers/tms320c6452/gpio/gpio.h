...
// RCI-ag-DrvGen Agent
// TMS320C6452 DSP GPIO Driver (Big-endian, Baremetal, GCC)
 ...
 *
 * Features:
 * - Banked architecture (2x16 pins, total 32)
 * - Direction control (DIR01)
 * - Output control via OUT/SET/CLR
 * - Input sampling (IN_DATA01)
 * - Per-pin edge-triggered interrupts (rising/falling)
 * - Per-bank interrupt enable (BINTEN)
 * - Interrupt status with W1C (INTSTAT01)
 * - PID readout for identification
 * - Initialization sequence hooks for pinmux/PSC
 */
...
void tmsgpio_platform_enable_psc(void);   // enable module clock
void tmsgpio_platform_disable_psc(void);  // disable module clock (low power)
void tmsgpio_platform_configure_pinmux(void); // ensure pins muxed to GPIO

// Initialize driver context. If base==0, uses TMS320C6452_GPIO_BASE_DEFAULT.
// If perform_platform_init is true, calls pinmux+PSC hooks.
int tmsgpio_init(tmsgpio_t *ctx, uintptr_t base, bool perform_platform_init);

// Read PID register (offset 0x00)
...
// Configure multiple pins: set bits in mask to desired dir (1=input, 0=output)
