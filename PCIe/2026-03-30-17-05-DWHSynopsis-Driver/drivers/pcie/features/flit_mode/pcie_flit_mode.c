...
pcie_status_t pcie_flit_query(const pcie_dbi_t* ctx, pcie_flit_info_t* info)
{
    ...
    if (pcie_cap == 0u) {
        info->flit_supported = false;
        info->pl64g_cap_version = 0u;
        info->pl64g_status = 0u;
        return PCIE_STATUS_UNSUPPORTED;
    }
    ...
    if (pl64g != 0u) {
        uint32_t hdr = pcie_read32(ctx, pl64g + 0x0u);
        info->pl64g_cap_version = pl64g_version_from_hdr(hdr);
        info->pl64g_status = pcie_read32(ctx, pl64g + 0xCu);
    } else {
        info->pl64g_cap_version = 0u;
        info->pl64g_status = 0u;
    }
...
pcie_status_t pcie_flit_enable(pcie_dbi_t* ctx, bool enable, uint32_t timeout_us)
{
    ...
    /* Quiesce: platform-specific. Here we simply program LC3 and poll Status2. */
    uint32_t lc3s3 = pcie_read32(ctx, pcie_cap + PCIE_CAP_REG_LINK_CTRL3_STAT3);
    uint32_t lc3 = lc3s3 & 0xFFFFu;

    if (enable) { lc3 |= LINK_CTRL3_FLIT_MODE_ENABLE_MASK; }
    else { lc3 &= ~LINK_CTRL3_FLIT_MODE_ENABLE_MASK; }

    uint32_t new_lc3s3 = (lc3s3 & 0xFFFF0000u) | (lc3 & 0xFFFFu);
    pcie_write32(ctx, pcie_cap + PCIE_CAP_REG_LINK_CTRL3_STAT3, new_lc3s3);

    /* Poll status2 for active bit matching requested state */
    uint32_t waited = 0u;
