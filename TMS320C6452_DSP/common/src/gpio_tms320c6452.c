/*
 * TMS320C6452 GPIO Driver - Source (Common)
 * Environment: Bare-metal, big-endian, GCC
 * Controller base: 0xA1008000
 *
 * Design:
 * - Deterministic 32-bit MMIO access
 * - No dynamic memory; no undefined behavior
 * - Atomic output operations via SET_DATA01 / CLR_DATA01
 * - Interrupt config via SET/CLR RIS/FAL, bank enable via BINTEN
 */
#include <stddef.h>
#include <stdint.h>
#include <stdbool.h>
...
{
    /* Platform should override to enable PSC/LPSC; default assume already enabled */
    return true;
}
...
{
    (void)pin;
    /* Platform should override to set pinmux; default assume already muxed to GPIO */
    return true;
}
...
gpio_status_t gpio_init(uintptr_t base)
{
    ...
    {
        return GPIO_STATUS_HW_MISMATCH; /* Clock not enabled */
    }

    (void)mmio_read32(GPIO_REG_PID_OFFSET); /* Probe */
    g_initialized = true;
...
gpio_status_t gpio_write_mask(uint32_t set_mask, uint32_t clear_mask)
{
    ...
    /* Write clear first, then set, to achieve final state deterministically */
    if (clear_mask != 0U)
    {
...
gpio_status_t gpio_isr_service(uint32_t bank, uint32_t *cleared_mask_out)
{
    ...
    if (to_clear != 0U)
    {
        mmio_write32(GPIO_REG_INTSTAT01_OFFSET, to_clear); /* W1C */
    }
